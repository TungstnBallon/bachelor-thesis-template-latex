...
Inputs and outputs are instances of classes that implement the interface \Verb[breakanywhere]|IOTypeImplementation|.
...
\begin{figure}
	\begin{plantuml}
		...
		@enduml
	\end{plantuml}
	\caption{The general structure of the Jayvee interpreter. Many subclasses and implementations have been omitted for readability}
...
\begin{figure}
	\begin{plantuml}
		...
		@enduml
	\end{plantuml}
	\caption{An activity diagram overview of the interpreter.} %TODO klären
...
The most direct approach would be a new implementation of the Arrow specification.
...
This implementation is comparable to those in other languages regarding supported data types, but it lags behind with support for other Apache frameworks \autocite{arrow:status}, which takes away one of Arrow's strengths.

\subsubsection{Polars}
\label{subsection:polars}
As described in \autoref{section:polars}, Polars is a high level library built on top of Apache Arrow, which also offers useful abstractions such as \Verb|DataFrames|, that hide the Arrow data types.
...
For these reasons, we choose to build the new table implementation on top of Polars.



\section{Type conversion}
\label{section:type_conversion}
Jayvee has a concept called \Verb|ValueType|, that represent Jayvee types, e.g. text, boolean or decimal. %FIXME citation
They are useful, because Jayvee types do not map directly to TypeScript types.
For example, when a value inside a table has the TypeScript type \Verb|number|, a \Verb|ValueType| denotes whether the value is a Jayvee \Verb|integer| or a Jayvee \Verb|decimal|.

Polars has a similar system, except it's called Verb|DataType|.
...
\begin{enumerate}
	\item \emph{Replace} \Verb|ValueType|s with \Verb|DataType|s.
	      We decided against this approach, because it enforces the Polars implementation at compile time, which breaks the feature toggle requirement.
	\item \emph{Convert} between \Verb|ValueType|s and \Verb|DataType|s when needed.
	      The TypeScript implementation can ignore the conversion and keep using \Verb|ValueType|s as before.
...
We move all the functionality from \Verb{Table} to \Verb{TsTable} and create a new \emph{abstract} class \Verb|Table|.
%TODO before / after klassendiagram

\subsection{abstract Table}
This class implements the \Verb|IOTypeImplementation<IOType.TABLE>| interface to be allowed as an input or output for blocks.
...
Polars' \Verb|DataFrame| already has table functionality, such as the ability to add, remove and transform columns.
However, \Verb|DataFrame|'s methods aren't the same as those required by \Verb|Table|.
To solve this, we create a wrapper class \Verb|PolarsTable|, that extends \Verb|Table| and thereby implements \Verb|IOTypeImplementation<IOType.TABLE>|, allowing \Verb|PolarsTable| to be used as an input or output for block executors.
...
One of the negative consequences is that adding a column into a \Verb|DataFrame| object is, that the column's \Verb|ValueType| is lost, because the \Verb|DataFrame| uses Polars' \Verb|DataType|s.
This is problematic, because the class \Verb|TableColumn| expects a \Verb|ValueType| for its constructor.

The solution is to convert the Polars \Verb|DataType| of a column to a \Verb|ValueType| using the conversion mechanism in \autoref{subsection:TypeConversion}.
...
%FIXME dieser absatz muss aufgebrochend werden
...
\Verb|DataFrame|'s method \Verb|getColumn(...)| returns an instance of the Polars class \Verb|Series|, which means that \Verb[breakbefore=o]|Series| %FIXME bad rendering in PDF
represents a column in Polars.
...
It should be noted, that \Verb|T| does not denote the actual type of the values stored in the column, as would be the case for \Verb|Array<T>|.
...
% At one point during development, \Verb|at(...)| was also implemented for the class \Verb|PolarsTableColumn|, but it was unused and thus \Verb|at(...)| was made exclusive to the class \Verb|TsTableColumn|. %FIXME decide whether to use this
...
\subsection{Overview of the new table implementation}
\autoref{fig:current_sit} visualizes the table architecture described in the previous subsections.
\begin{figure}[H]
	\begin{plantuml}
		@startuml
		hide members
...
The Jayvee interpreter uses the method \Verb|createBlockExecutor(...)| to get an implementation of the interface \Verb|BlockExecutor|.
...
Blocks with the type \Verb|FileToTableInterpreter| receive a \Verb|BinaryFile|, which contains a TypeScript \Verb|ArrayBuffer| and converts that into a table.
This combines the block types \Verb|TextFileInterpreter|, \Verb|CSVInterpreter| and \Verb|TableInterpreter|.

Merging these block types allows us to utilize Polars' builtin \ac{CSV} parsing functionality, instead of relying on the \Verb|fast-csv| library, like \Verb|CSVInterpreter|, which means less indirection when reading a \ac{CSV} file.

\subsubsection{Adding a block type}
To add this block type to the Jayvee language, it's definition is put in \Verb|FileToTableInterpreter.jv| (refer to \autoref{tab:src} for the full path).
...
\begin{figure}
	...
	\end{subfigure}
	\caption{Jayvee pipelines using only original block types, \Verb{FileToTableInterpreter} or \Verb{LocalFileToTableInterpreter}}
...
We consider this to be a non-optimal representation of the data and improve it in \autoref{arch:sqlite:rust}. %FIXME this shows up as Figure X.X.X
...
For this reason, we choose Rust to implement sqlite-loader-lib (see \autoref{fig:uml:rust_components}).
...
This ensures, that the existing TypeScript implementation remains available.
...
Jayvee currently has ~32 evaluator operators. %FIXME: decide whether to be imprecise or not. I counted 32. Mention, that this is my count.
