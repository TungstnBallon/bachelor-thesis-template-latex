...
We add an alternative table implemetation, based on the Apache Arrow framework \autoref{section:arrow}, to the interpreter.
...
To this end, we have implemented the typeguards \Verb|isPolars()| and \Verb|isTypescript()|.
...
The method \Verb|TsTable.addColumn(...)| preserves the old behaviour.
...
\Verb|withColumnFromInternal| returns a new Table with the column resulting from the passed Polars expression added.
...
\Verb|PolarsTable| requires a \Verb|ValueTypeProvider| to be passed to its contructor (see \autoref{subsubsection:polarstablecolumn}).
...
The facilitate automatic optimization and parallelization \autocite{polars:docs:expr}.
...
Compared to the old \Verb|TableColumn| (\autoref{fig:uml:old_tablecolumn}), the new abtract class no longer defines how the values of the column are saved.
...
Running the command \Verb|npm run generate| creates the necessary files for this blocktype to be used in the Jayvee interpreter.

We also create an executor for the new blocktype (\autoref {fig:uml:filetotableinterpreter}).
...
The abstract \Verb|SQLiteLoaderExecutor| (\autoref{fig:uml:sqliteloader}) offers a general implementation, utilising methods defined in the abstract \Verb|Table| class.
