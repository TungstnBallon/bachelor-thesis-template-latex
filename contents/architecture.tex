...
\Verb|addColumn| is replaced in favor of \Verb|withColumn|, the concrete differences are explained in \autoref{subssubsection:impl:old_vs_new}.
Typeguards to distinguish the implementations, \Verb|isPolars| and \Verb|isTypescript|, were added.
Also, we replaced the \Verb|number_of_rows| property with getter method using the \Verb|get| keyword.
...
For example \mintinline{typescript}{abstract clone(): Table} becomes \mintinline{typescript}{override clone(): PolarsTable}.
...
to be passed to its contructor. See \autoref{subsubsection:polarstablecolumn}.
See \autoref{fig:uml:polarstable} for the class diagram.
