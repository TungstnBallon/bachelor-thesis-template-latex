...
This section will provide an overview of how the interpreter executes a Jayvee pipeline, illustrated by \autoref{fig:uml:overview}.
...
Polars is a high level api built on top of apache arrows, but it also offers useful abstractions like DataFrames, that hide the Arrow data types.
These abstractions make it easier for developers unfamiliar with Arrow to contribute to Jayvee.
Polars is written in rust, but offers a TypeScript \ac{API}, allowing us %FIXME: us
...
The approach to executing transforms no longer is to compute all the values of the new column, but to create a Polars expression, and return it to \Verb|PolarsTableTransformerExecutor|.
...
Compared to the old \Verb|TableColumn| (\autoref{fig:uml:old_tablecolumn}), the new abstract class no longer defines how the values of the column are saved.
...
In addition to the abstract methods from \Verb|TableColumn|, \Verb|TsTableColumn| also has methods that allow users to use the wrapped array functionality, \Verb|push|, \Verb|at| and \Verb|drop|.
...
Classes extending \Verb|AbstractBlockExecutor|, instead of implementing \Verb|BlockExecutor|, benefit from its general implementation of \Verb|execute|, that leaves the execution behavior to \Verb|doExecute(...)|.
...
This block receives a \Verb|BinaryFile|, which contains a JavaScript \Verb|ArrayBuffer| and converts that into a Table, combining TextFileInterpreter, CSVInterpreter and TableInterpreter.
Unifying these blocks allows us to utilize Polars' builtin \ac{CSV} parsing functionality, instead of relying on the \Verb|fast-csv| library like \Verb|CSVInterpreter|.
...
The process of adding the blocktype to jayvee is the same as described in \autoref{subsection:FileToTableInterpreter},
