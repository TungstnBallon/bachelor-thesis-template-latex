...
used Apache Arrow to build \emph{SHMEM-ML}, a machine learning library.
...
This is known as \emph{columnar} or \emph{column-storage} \autocite{Floratou2019}.
See Figure~\autoref{fig:row_v_col} for a visual comparison.
\begin{figure}[h]
	\begin{center}
...
Apache Arrow is often used synonymously with the memory format it defines.
...
In the Arrow specification \autocite{arrow:spec}, a table is called \emph{record batch}.
They are comprised of Arrow arrays which represent columns.
These Arrow arrays are different from TypeScript arrays, as they cannot contain different data types.
...
Arrow has a more granular type system than TypeScript. This allows for a more precise control of memory.
Arrow types can have parameters.
For example instead of TypeScript's type \Verb|number|, Arrow has (un)signed integers with a bit width parameter,
...
Additionally, the Arrow specification defines an \ac{IPC} file format, that allows saving Arrow record batches on disk.

At the time of writing this thesis, there are 48 projects powered by Apache Arrow \autocite{arrow:projects}.
...
\begin{figure}[h]
	...
	\end{center}
	\caption{Illustration demonstrating Arrow enhances interoperability between processes \autocite{Ahmad2020}}
	\label{fig:arrow_com}
\end{figure}
...
\emph{Polars} is built on top of the Rust implementation of the Arrow specification.
It incorporates convenient abstractions such as \Verb|DataFrame| and \Verb|Series|.
Additionally, it uses NAPI-RS to provide a TypeScript \ac{API} that preserves the performance of the rust library.
