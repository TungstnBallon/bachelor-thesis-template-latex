...
In general, we used methods such as \Verb|map(...)| over for loops because they are, in our expierience, less error prone, and allow for cleaner code.
...
fromPolarsDType(dtype: polars.DataType): ValueType
	\end{minted}
\end{center}
It uses the method \Verb|DataType.equals(\dots)| instead of \Verb|===| for a more accurate comparison of DataTypes.
...
To narrow down the return type, \Verb|any|, we expand the existing typeguards, implemented in \Verb|typeguards.ts|, to handle inputs with the type \Verb|unknown|.
