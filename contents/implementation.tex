...
In this chapter, we explain the implementation of the architecture described in \ref{chapter:Architecture}.
...
As described in \ref{section:type_conversion}, we implement a conversion mechanism between Polars' DataTypes and Jayvee's ValueTypes.
...
\ref{tab:internalvaluerepresentation} contains a of internal types and the mechanism their typeguard uses.


\section{Table}
\label{arch:table}
The overview of existing classes and their relations is given in \ref{section:new_table}. \\
...
% }
% 	\end{minted}
% 	\caption{The source code of the method \Verb{generateDropTableStatement}.}
% 	\label{lst:table:drop_table}
% \end{listing}

\begin{itemize}
	\item To reduce the code in the source file, \Verb|table.ts| (\ref{tab:src}) %TODO explain source file links in general
	...
	\item Because of the modularization requirement and the fact, that the Polars and TypeScript implementations of the method \Verb|generateDropTableStatement| are identical, we implement it as a non abstract method in \Verb|Table| (\ref{lst:table:drop_table}). %FIXME "non abstract" wording
...
The construction of this uses the conversion method from \ref{impl:types:dtov} using the class \Verb|PolarsTables|'s attribute, \Verb|valueTypeProvider|
...
\Verb|doExecute| merely preprocesses the block's properties, leaving the concrete algorithm to \Verb|TableInterpreterExecutor|'s subclasses (\ref{lst:tableInterpreter:doExecute}).
\begin{listing}
	\begin{minted}{python}
...
Because we manipulated the selection process in \ref{subsection:createBlockExecutor}, the interpreter is looking for \Verb|PolarsTableInterpreter| instead of \Verb|TableInterpreter| if the \Verb|--use-polars| \ac{CLI} flag is set.
