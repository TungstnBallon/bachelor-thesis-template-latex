...
\begin{description}
	...
	\item[openness] The dataset must have a licence conformant with the Open Definition \autocite{opendefinition}.
	      \textcite{opendefinition:licenses} provides a list of recommended and conformant licenses.
\end{description}
...
\begin{description}
	...
	\item[plrs] Polars-Rusqlite.
	      ...
	      Enables pl, plus the \Verb|RustSQLiteLoaderExecutor| executor and the the \Verb|sqlite-loader-lib| library (see \autoref{arch:sqlite:rust}).
	\item{plobrs} Polars-One-Block-Rusqlite.
	      ...
	      This backend not usable for every pipeline.
...
\autoref{fig:uml:pick-jv-model} outlines the process by which the evaluation tool selects the correct source file.
\begin{figure}
	\begin{plantuml}
		@startuml
...
%NOTE
We considered it simplest do construct and execute a series of shell commands, simpler than using 3rd-party libraries. scince running the interpreter has to be a shell command anyway.
...
\begin{description}
	...
	\item[plob] The plob backend performs better than the baseline on all tested pipelines and inputs.
	      This indicates, that the blocktype \Verb|LocalFileToTableExtractor| (\autoref{arch:execs:localfiletotableextractor}) is a more effective method of parsing local \ac{CSV} data, than the existing blocktypes. %TODO link diaram
...
\begin{enumerate}
	...
	\item[FR-2] \textbf{columnar} Polars implements the Apache Arrow specifictation, which is a columnar format.
	...
	\item[FR-4] \textbf{compatibility} The electric vehicles example completes without errors.
	      ...
	      Consequiently, this requrement remains unfulfilled.
...
It is our contention, that all functional requirements, with the exception of compatibility, have been met.
...
\begin{enumerate}
	...
	\item[NFR-2] \textbf{codestyle} \Verb|npm run lint| only throws erros for files out of the scope of this thesis.
