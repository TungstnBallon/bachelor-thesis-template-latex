...
In this section will compare the execution durations of the configurations defined in \ref{section:parameters}.
...
\ref{fig:plot_no_transforms} shows the average execution time for each backend, \ref{tab:eval:none} contains the concrete values.

It is evident, that, the majority of backends, with the exception of pl, are faster, compared the baseline ts.
plobrs is the most expedient backend for all tested input sizes.
Furthermore, the mean slope of the ts backend is $12$ times larger than that of the plobrs backend.
% We also observe, that the plrs optimization is, on average, $1,64$ times faster, that the backend pl.

It is somewhat unexpected, that the pl backend has the longest runtime.
The pl backend was observed to be $1,33$ times slower than the the baseline ts backend.
To investigate this, the debug outputs of one run, of both these configurations (\ref{tab:cmp:sqlite}) were compared.
It was observed, that the duration of the block \Verb|LiquorLoader| approximately doubled when the backend pl was enabled.
This was considered to be the cause. %



All backends, execpt pl, are considered to be successful optimisations for a pipeline with no transforms.

\begin{figure}
	\begin{tikzpicture}
		\begin{axis}[
				title = {Runtime with no transforms},
				xlabel = {lines},
				ylabel = {time in ms},
...
\ref{fig:plot_so_transforms} illustrates the results for a pipeline with some transforms.
The precise values are listed in \ref{tab:eval:some}.

The results for a pipeline with some transforms are, on the whole, consistent with those presented in \ref{eval:num:none}.

However, the average execution duration for the pl backend is closer to that of the ts baseline, with ts being $1,09$ times as fast as pl., compared to $1,33$ in \ref{eval:num:none}
We attribute this decrease to the inclusion of transforms in the evaluation pipeline.
Consequently, the block executor \Verb|PolarsTableTransformerExecutor| is evaluated to be faster than the \Verb|TsTableTansformerExecutor|.
It is anticipated, that the pl backend will, for a pipeline with a large enough number of transforms, be faster than the ts baseline.
